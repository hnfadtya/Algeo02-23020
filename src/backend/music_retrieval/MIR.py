...
def process_midi (audio_path):
    ...
    for track in mid.tracks:
        for msg in track:
            #print (msg)
            if msg.type == 'note_on' :  # Sementara kita ambil dari smua channel
...
def extract_features(notes):
    ...
    # Filter pitch untuk memastikan hanya nilai non-negatif yang diproses
    if len(pitches) == 0 or np.any(pitches < 0):
        return np.concatenate([np.zeros(128), np.zeros(255), np.zeros(255)])

    # Histogram Absolute Tone (ATB)
    hist_atb = np.bincount(pitches, minlength=128) / len(pitches)

    # Histogram Relative Tone (RTB)
    selisih_rtb = np.diff(pitches)
    hist_rtb = np.bincount(selisih_rtb + 127, minlength=255) / len(selisih_rtb) if len(selisih_rtb) > 0 else np.zeros(255)

    # Histogram First Tone (FTB)
    first_tone = pitches[0]
    selisih_ftb = pitches - first_tone
    hist_ftb = np.bincount(selisih_ftb + 127, minlength=255) / len(selisih_ftb) if len(selisih_ftb) > 0 else np.zeros(255)
...
def query_by_humming(midi_query_path, vektor_database):
...
def print_top_similar_song(results, top_n = 10):
    ...
    # Urutkan file berdasarkan skor rata-rata, dari tertinggi ke terendah
